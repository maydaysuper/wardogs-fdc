...
- **v0.12.0 原生游戏画面采集**：纯视觉导航不再依赖 OBS Projector/预览窗口。程序直接寻找 WARDOGS 游戏窗口并抓取客户区像素。
- 自动采集顺序改为 **NativeWindow → ScreenCopy → PrintWindow**。原生窗口帧异常/黑帧时自动回退，不需要用户运行 OBS。
- 前方道路视觉、速度 HUD OCR、游戏地图视觉配准、目标识别都直接消费游戏画面；旧 `CaptureWindowTitleContains` 仅为兼容历史 settings.json 保留，不再作为纯视觉数据源。
- AI/视觉模型与采集层分离：Windows 原生采集负责稳定取得像素，本地视觉/后续 ONNX 道路模型负责识别道路、运动、障碍和路况。

- **v0.11.1 游戏内全局快捷键**：无需 Alt+Tab。程序后台运行时可直接在游戏中按 `Ctrl+Alt+F8` 截图识别当前位置、`Ctrl+Alt+F9` 截图识别目的地并规划、`Ctrl+Alt+F10` 启动/停止实时导航。快捷键只触发屏幕读取和本地导航，不向游戏发送输入。\n- **v0.11.0 视觉连续定位**：导航初始化改为“两次截图”工作流——先截取当前位置，再截取目的地；进入驾驶后可停止持续读取当前位置坐标 OCR。
...
- **v0.10.0 UI / AI / 性能 / 兼容性升级**：统一中文深色 UI、顶部运行状态栏、低占用/平衡/实时三档性能模式；旧版曾支持 OBS 独立采集源，v0.12 起纯视觉链路已改为直接游戏窗口采集。
...
4. 首次校准后不必再切回软件：保持游戏前台，按 `Ctrl+Alt+F8` 获取当前位置，再按 `Ctrl+Alt+F9` 获取目的地并规划；关闭地图回到驾驶画面后按 `Ctrl+Alt+F10` 开始实时导航。也可以使用软件里的①/②按钮完成同样操作。
